...
PYBIND11_MODULE(py_sirius, m)
{
    ...
    py::class_<DFT_ground_state>(m, "DFT_ground_state")
        ...
        .def("find", [](DFT_ground_state& dft, double potential_tol, double energy_tol, int num_dft_iter, bool write_state, bool mix) {
            json js = dft.find(potential_tol, energy_tol, num_dft_iter, write_state, mix);
            return pj_convert(js);
        }, "potential_tol"_a, "energy_tol"_a, "num_dft_iter"_a, "write_state"_a, "mix"_a = true)
    ...
    py::class_<Free_atom>(m, "Free_atom")
        .def(py::init<Simulation_context&, std::string>())
